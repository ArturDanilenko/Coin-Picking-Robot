...
void main(void)
{
	int j, rc3,rc10,rc2,rc9,channel3, channel10,channel9,channel2, PerimeterDetected = 1, CleaningFlag = 1, num = 0;
	long int count;
	float T, f;
	ConfigPins();
	initUART(115200);
...
//	printf("%i|",num); uncomment alter jed
    ...
    //	printf("|ADC[3]=%d.%04dV|", channel3/10000, channel3%10000);
		...
		channel9 = getReading(9); 
		if(channel3>24000)perimeterdetected=1;
		if(channel9>24000)perimeterdetected=1;
	//	printf("%i|",num);
	//	printf("%i|%i|%i",perimeterdetected,coindetected,num); uncomment
		count=GetPeriod(100);
		if(count>0)
		{
			T=count/(F_CPU*100.0);
			f=1/T;
		//	eputs("f="); uncomment eputs and printernum later jed
		//	PrintNumber(f, 10, 7);
		//	eputs("Hz, count=");
